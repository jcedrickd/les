...
<meta http-equiv="content-type" content="text/html; charset=UTF-8">
	...
	<script>
function addTab(title, url, mainmenu,mainmenu_title){
    if ($('#content1').tabs('exists', mainmenu)){
        $('#content1').tabs('select', mainmenu);
    }else{
        var content = "<div title='"+mainmenu_title+"' id='"+mainmenu+"m' class='easyui-tabs' style='width:100%;'></div>";
        $('#content1').tabs('add',{
...
$result_check_submenu=sybase_query("SELECT mcategory_sub,mmain_name,url FROM menu_main WHERE mcategory_ctrlno=$nest1[mcategory_ctrlno] ORDER BY mmain_name ASC");
	...
	ORDER BY mmain_name ASC");
		...
		echo '<li><a href="#" onClick="addTab('."'".$nest3['mmain_name']."'".','."'"."http://".$_SERVER['SERVER_NAME']."/".$nest3['url']."'".','."'".str_replace(' ','_',$nest1['mcat_name'])."'".','."'".$nest1['mcat_name']."'".');">'.$nest3['mmain_name'].'</a></li>';
		}
		echo '</ul></li>';//li for page
	}
