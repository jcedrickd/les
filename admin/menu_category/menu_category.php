...
foreach ($DelItem as $key=>$val){
	$DelEach="DELETE FROM menu_category WHERE mcategory_ctrlno=$val";
...
<?php 
include($_SERVER['DOCUMENT_ROOT'].'les/includes/sybase_connect.php'); 
$query=sybase_query("SELECT * FROM menu_category");
$numrows=sybase_num_rows($query);
	while($row=sybase_fetch_array($query)){
?>
	<td style="<?php echo @$posted_table.' '.@$cancelled_table; ?>" class="hidable_column <?php echo @$access_update; ?>"><a class="hover_row" href="edit.php?getValiD=<?php echo @$row['mcategory_ctrlno'];?>">Edit</td>
	<td style="<?php echo @$posted_table.' '.@$cancelled_table; ?>" class="hidable_column <?php echo @$access_delete; ?>"><input type="checkbox" value="<?php echo @$row['mcategory_ctrlno']; ?>" name="checkbox[]" class="highlight_row" /></td>
	<td><?php echo @$row['mcategory_ctrlno']; ?></td>
