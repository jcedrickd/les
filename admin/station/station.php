...
foreach ($DelItem as $key=>$val){
	$DelEach="DELETE FROM branch WHERE branch_code='$val'";
...
?>
<b class="module">Station ID List</b>
...
<?php 
include($_SERVER['DOCUMENT_ROOT'].'les/includes/sybase_connect.php'); 
$query=sybase_query("SELECT * FROM branch");
$numrows=sybase_num_rows($query);
	while($row=sybase_fetch_array($query)){
?>
<tr>
	<td style="<?php echo @$posted_table.' '.@$cancelled_table; ?>" class="hidable_column <?php echo @$access_update; ?>"><a class="hover_row" href="edit.php?getValiD=<?php echo @$row['branch_code'];?>">Edit</td>
	<td style="<?php echo @$posted_table.' '.@$cancelled_table; ?>" class="hidable_column <?php echo @$access_delete; ?>"><input type="checkbox" value="<?php echo @$row['branch_code']; ?>" name="checkbox[]" class="highlight_row" /></td>
