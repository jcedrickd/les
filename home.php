...
<meta http-equiv="content-type" content="text/html; charset=UTF-8">
	...
	<script>
		function addTab(parent,title, url){
			if ($('#'+parent).tabs('exists', title)){
				$('#'+parent).tabs('select', title);
			} else {
		...
		/*
		function addNoSubTab(title, url){
			if ($('#content1').tabs('exists', title)){
				$('#content1').tabs('select', title);
			} else {
				var content = '<iframe src="'+url+'" class="iframe" scrolling="auto" frameborder="0" style="width:100%;height:754px;"></iframe>';
				$('#content1').tabs('add',{
					title:title,
					content:content,
					closable:true
                ...
                function addMainTab(id,title){
			if ($('#content1').tabs('exists', title)){
				$('#content1').tabs('select', title);
			} else {
				var content = '<div title="'+title+'" id="'+id+'" class="easyui-tabs" style="width:100%;"></div>';
				$('#content1').tabs('add',{
					title:title,
					content:content,
					closable:true
    ...
    <div class="navbar navbar-fixed-top navbar-inverse" role="navigation">
      <div class="container">
        <div class="navbar-header">
          ...
          <a class="navbar-brand" href="#" style="color:white;"><img width="20%" src="images/general/aai.png" />Logistics Enterprise Systems</a>
        </div>
        <div class="collapse navbar-collapse" style="float:right;">
          <ul class="nav navbar-nav">
			<li class=""><p style="color:#FFF; font-size; 14px;">Logged in as <?php echo @$_SESSION['fullname']; ?></p></li>
            <li class="active"><a style="font-size: 14px;" href="logout.php">Log Out</a></li>
...
<li class="active has-sub">
	<a onClick="addMainTab('."'".str_replace(' ','_',$nest1['mcat_name'])."'".','."'".$nest1['mcat_name']."'".');" href="#">'.$nest1['mcat_name'].'</a>
		<ul>';
...
$result_check_submenu=sybase_query("SELECT mcategory_sub,mmain_name,url FROM menu_main WHERE mcategory_ctrlno=$nest1[mcategory_ctrlno] ORDER BY mmain_name ASC");
	while($check_submenu=sybase_fetch_array($result_check_submenu)){
		...
		echo '<li><a href="#" onClick="addTab('."'".str_replace(' ','_',$nest1['mcat_name'])."'".','."'".$check_submenu['mmain_name']."'".','."'"."http://".$_SERVER['SERVER_NAME']."/".$check_submenu['url']."'".');">'.$check_submenu['mmain_name'].'</a></li>';
		}
	}
	...
	ORDER BY mmain_name ASC");
		...
		echo '<li><a href="#" onClick="addTab('."'".str_replace(' ','_',$nest1['mcat_name'])."'".','."'".$nest3['mmain_name']."'".','."'"."http://".$_SERVER['SERVER_NAME']."/".$nest3['url']."'".');">'.$nest3['mmain_name'].'</a></li>';
		}
		echo '</ul></li>';//li for page
	}
